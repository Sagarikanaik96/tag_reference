...
def execute(filters=None):
	...
	try:
		data = frappe.db.sql('''select `tabSales Invoice`.name,company,base_paid_amount,(base_grand_total-base_paid_amount) as due_amount,(base_paid_amount+(base_grand_total-base_paid_amount)) as due_amount,(base_paid_amount+(base_grand_total-base_paid_amount))*((`tabCompany`.tag_charges)/100) as approx_due,base_paid_amount*((`tabCompany`.tag_charges)/100) as total_due,is_pos from `tabSales Invoice`,`tabCompany` where `tabSales Invoice`.company=`tabCompany`.name and customer='tag'  %s ''' % condition, as_dict=True)        
		for d in data:
			time=frappe.db.sql(f''' select min(creation) as start_time,max(creation) as last_time from `tabSales Invoice` where customer!='tag' and company="{d.company}" group by company  ''',as_dict=True)
			row.append({"company":d.company , "name": d.name, "first_creation": time[0].start_time, "last_creation": time[0].last_time, "total_received":d.base_paid_amount, "total_pending":d.due_amount, "approx_due": d.approx_due, "total_due": d.total_due, "is_paid":d.is_pos,"button":"<a href='/app/query-report/Tag%20Invoice?company={d.company}'>View Company Report</a>" })
		return columns,row
