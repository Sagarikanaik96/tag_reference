...
def fetch_data_contd(filters=None):
    try:
        ...
        if user_data[0] == 'Hiring' or user_data[0] == 'Exclusive Hiring':
            sql = '''SELECT AED.employee AS emp_id, EMP.first_name AS first_name, EMP.last_name AS last_name, AED.employee_name as emp_name, AE.company AS company, AE.job_order AS job_order, AE.job_category AS job_title, JO.from_date AS start_date FROM `tabAssign Employee` AS AE,`tabAssign Employee Details` AS AED, tabEmployee AS EMP, `tabJob Order` AS JO  WHERE AED.approved = CASE WHEN JO.resumes_required=1 THEN 1 ELSE 0 END AND AE.name = AED.parent AND AE.hiring_organization = "{0}" AND AE.tag_status = "Approved" AND EMP.name = AED.employee AND JO.name = AE.job_order {1} ORDER BY AE.job_order, AE.company, EMP.first_name'''.format(user_data[1], condition)
        elif user_data[0] == 'TAG' or frappe.session.user == 'Administrator':
            sql = '''SELECT AED.employee AS emp_id, EMP.first_name AS first_name, EMP.last_name AS last_name, AED.employee_name as emp_name, AE.company AS company, AE.job_order AS job_order, AE.job_category AS job_title, JO.from_date AS start_date FROM `tabAssign Employee` AS AE,`tabAssign Employee Details` AS AED, tabEmployee AS EMP, `tabJob Order` AS JO  WHERE AED.approved = CASE WHEN JO.resumes_required=1 THEN 1 ELSE 0 END AND AE.name = AED.parent AND AE.tag_status = "Approved" AND EMP.name = AED.employee AND JO.name = AE.job_order {0} ORDER BY AE.job_order, AE.company, EMP.first_name'''.format(condition)
