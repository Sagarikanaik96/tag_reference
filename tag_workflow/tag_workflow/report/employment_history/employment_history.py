...
def execute(filters = None):
    ...
    if filters.get('total_hours'):
        data = [i for i in data if 'total_hours' in i and i['total_hours'] == float(filters.get('total_hours'))]
...
def get_condition(filters):
    try:
        ...
        if(filters.get('company')):
            condition += " and AED.company = '{0}'".format(filters.get('company'))
...
def fetch_data(filters=None):
    try:
        ...
        for i in data:
            sql = '''SELECT total_hours, no_show, non_satisfactory, dnr FROM tabTimesheet WHERE workflow_state = 'Approved' AND employee = "{0}" AND job_order_detail = "{1}"'''.format(i['emp_id'], i['job_order'])
...
def fetch_data_contd(filters=None):
    try:
        ...
        if user_data[0] == 'Hiring':
            sql = '''SELECT AED.employee AS emp_id, EMP.first_name AS first_name, EMP.last_name AS last_name, AED.employee_name as emp_name, AED.company AS company, AE.job_order AS job_order, AE.job_category AS job_title, JO.from_date AS start_date FROM `tabAssign Employee` AS AE,`tabAssign Employee Details` AS AED, tabEmployee AS EMP, `tabJob Order` AS JO  WHERE AED.approved = CASE WHEN JO.resumes_required=1 THEN 1 ELSE 0 END AND AE.name = AED.parent AND AE.hiring_organization = "{0}" AND AE.tag_status = "Approved" AND EMP.name = AED.employee AND JO.name = AE.job_order {1}'''.format(user_data[1], condition)
        elif user_data[0] == 'TAG' or frappe.session.user == 'Administrator':
            sql = '''SELECT AED.employee AS emp_id, EMP.first_name AS first_name, EMP.last_name AS last_name, AED.employee_name as emp_name, AED.company AS company, AE.job_order AS job_order, AE.job_category AS job_title, JO.from_date AS start_date FROM `tabAssign Employee` AS AE,`tabAssign Employee Details` AS AED, tabEmployee AS EMP, `tabJob Order` AS JO  WHERE AED.approved = CASE WHEN JO.resumes_required=1 THEN 1 ELSE 0 END AND AE.name = AED.parent AND AE.tag_status = "Approved" AND EMP.name = AED.employee AND JO.name = AE.job_order {0}'''.format(condition)
