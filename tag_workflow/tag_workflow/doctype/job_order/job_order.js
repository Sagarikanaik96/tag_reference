        ...
        return new Promise(function (resolve, reject) {
          ...
          if (frm.doc.contact_number) {
            profile_html =
              ...
              "<br><b>Start Time: </b>" +
              frm.doc.job_start_time.slice(0, -3) +
              "<br><b>Job Site: </b>" +
              frm.doc.job_site +
              ...
              "<br><b>Base Price: </b>$" +
              frm.doc.rate.toFixed(2) +
              "<br><b>Rate Increase: </b>$" +
              (frm.doc.per_hour - frm.doc.rate).toFixed(2) +
              "<br><b>Additional Flat Rate: </b>$" +
              frm.doc.flat_rate.toFixed(2) +
              "<br><b>Total Per Hour Rate: </b>$" +
              frm.doc.per_hour.toFixed(2) +
              "</span>";
          } else {
            profile_html =
              "<span style='font-size: 14px;'>" +
  ...
  frappe.call({
    ...
    callback: function (r) {
      if (r.message != "success") {
        ...
        frappe.call({
          method:
            "tag_workflow.tag_workflow.doctype.job_order.job_order.submit_headcount",
          args: {
            job_order: frm.doc.name,
            staff_company: frappe.boot.tag.tag_user_info.company,
          },
          callback: function (s) {
            if (s.message) {
              ...
              let final_count =
                claims_left > remaining_approved_emp
                  ? remaining_approved_emp
                  : claims_left;
              doc.job_order = frm.doc.name;
  ...
  frappe.db.get_value(
    ...
    function (r) {
      if (
        ...
        (frm.doc.__islocal != 1 &&
          frappe.boot.tag.tag_user_info.company_type == "Staffing" &&
          frm.doc.company_type == "Exclusive" &&
...
function staff_company_read_only(frm) {
  if (
    frm.doc.__islocal != 1 &&
    frm.doc.company_type == "Non Exclusive" &&
    frappe.boot.tag.tag_user_info.company_type == "Staffing"
  ) {
    let myStringArray = [
      "company",
...
function order_claimed_contd(result, frm) {
  ...
  if (
    total_approved_emp == frm.doc.no_of_workers &&
    frm.doc.order_status != "Completed"
  ) {
    frm.remove_custom_button("Claim Order");
...
function assigned_emp_comp(frm) {
  frappe.call({
    method:
      "tag_workflow.tag_workflow.doctype.job_order.job_order.assigned_emp_comp",
    args: {
      job_order: frm.doc.name,
    },
    callback: function (r) {
      if (
        r.message.length > 0 &&
        !r.message.includes(frappe.boot.tag.tag_user_info.company)
      ) {
    ...
    frappe.call({
      ...
      callback: function (s) {
        if (s.message) {
          ...
          let remaining_approved_emp = s.message[2];
          if (
            remaining_approved_emp != 0 &&
            frm.doc.order_status != "Completed" &&
            total_claims < frm.doc.no_of_workers
          ) {
            reclaim_button(frm);
          }
        }
...
function staff_assign_button_claims(frm, r) {
  if (r.message[0] == "success1") {
    ...
    if (
      frm.doc.no_of_workers - frm.doc.worker_filled != 0 &&
      r.message != "success2" &&
...
function reclaim_button(frm) {
  frappe.call({
    method:
      "tag_workflow.tag_workflow.doctype.job_order.job_order.claim_order_updated_by",
    args: {
