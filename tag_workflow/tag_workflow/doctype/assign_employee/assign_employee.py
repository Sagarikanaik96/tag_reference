    ...
    try:
        ...
        for index ,i in enumerate(employee_lis):
            if index >= 1:
                ...
                value =value+i
        if not job_category:
            sql = """ select name, employee_name, street_address, city, state, zip from `tabEmployee` where company = '{0}' and status = 'Active' and name NOT IN (select parent from `tabBlocked Employees` where blocked_from = '{2}') and name NOT IN (select parent from `tabDNR`  where dnr = '{2}') and name NOT IN ('{3}') and employee_name like '%%{3}%%' and name NOT IN (select e.employee from `tabAssign Employee Details` e inner join `tabAssign Employee` a where a.name = e.parent and a.hiring_organization = '{2}' and a.job_order not in (select name from `tabJob Order` where company = '{2}' and order_status = "Completed")) """.format(emp_company, company, value, '%s' % txt)
        else:
            sql = """ select name, employee_name, street_address, city, state, zip from `tabEmployee` where company = '{0}' and status = 'Active' and name in (select parent from `tabJob Category` where job_category = '{1}' or job_category IS NULL) and name NOT IN (select parent from `tabBlocked Employees` where blocked_from = '{2}') and name NOT IN (select parent from `tabDNR`  where dnr = '{2}') and name NOT IN ('{3}') and employee_name like '%%{3}%%' and name NOT IN (select e.employee from `tabAssign Employee Details` e inner join `tabAssign Employee` a where a.name = e.parent and a.hiring_organization = '{2}' and a.job_order not in (select name from `tabJob Order` where company = '{2}' and order_status = "Completed"))  """.format(emp_company, job_category, company, value, '%s' % txt)
