frappe.pages['print-invoice'].on_page_load = function (wrapper) {
	...
	$(wrapper).bind('show', () => {
		localStorage.removeItem('invoice_view');
		localStorage.removeItem('current_invoice_view');
...
function set_button(print_view, frm){
	if(!frm.doc.job_order){
		$('#summary_view, #detailed_view').hide();
	}else{
		$('#summary_view, #detailed_view').show();
		let default_button = print_view.get_view() == 'Invoice Detailed View' ? 'Summary View' : 'Detailed View';
		if(default_button=='Summary View'){
			$('#detailed_view').html(default_button)
			$('#detailed_view').attr('id', frappe.scrub(default_button))
		}else{
			$('#summary_view').html(default_button)
			$('#summary_view').attr('id', frappe.scrub(default_button))
		}
	}
}
...
frappe.ui.form.PrintView = class {
	...
	async setup_toolbar() {
		...
		await frappe.model.with_doc(doctype, docname, () => {
			...
			if (frm.job_order) {
				let view = this.get_view() == 'Invoice Detailed View' ? 'Summary View' : 'Detailed View';
				localStorage.setItem('current_invoice_view', this.get_view())
				this.page.add_button(
					__(view),
					() => {
						let storage_view = localStorage.getItem('invoice_view')
						if (!storage_view) {
							localStorage.setItem('invoice_view', view);
						}
						this.refresh_print_format();
					}
				).attr('id', frappe.scrub(view)).addClass('btn btn-primary');
			}
		});
		...
		this.page.add_button(
			__('Full Page'),
			() => {
				let current_view = localStorage.getItem('current_invoice_view')
				this.render_page('/printview?', false, current_view?current_view:'')
			},
			{ icon: 'full-page' }
		);
		...
		this.page.add_button(
			...
			() => {
				let current_view = localStorage.getItem('current_invoice_view');
				this.render_page('/api/method/frappe.utils.print_format.download_pdf?', false, current_view?current_view:'')
			},
			{ icon: 'small-file' }
		);
	}
	...
	refresh_print_format() {
		...
		this.preview();
		let view = localStorage.getItem('invoice_view');
		if (view) {
			let new_view = view == 'Detailed View' ? 'Summary View' : 'Detailed View'
			localStorage.setItem('invoice_view', new_view);
			localStorage.setItem('current_invoice_view', view == 'Detailed View' ? 'Invoice Detailed View' : 'Sales Invoice Report')
			$('#' + frappe.scrub(view)).html(new_view);
			$('#' + frappe.scrub(view)).attr('id', frappe.scrub(new_view))
		}
	}
	...
	render_page(method, printit = false, view='') {
		let w = window.open(
			frappe.urllib.get_full_url(
				...
				'&format=' +
				encodeURIComponent(view?view:this.selected_format()) +
				'&no_letterhead=' +
	...
	selected_format() {
		let invoice_view = localStorage.getItem('invoice_view');
		let route = frappe.get_route();
		let doctype = route[1];
		let docname = route[2];
		let frm = frappe.get_doc(doctype, docname);
		if(frm.job_order){
			if (!invoice_view) {
				...
				return invoice_view == 'Detailed View' ? 'Invoice Detailed View' : 'Sales Invoice Report'
			}
		}else{
			return 'Sales Invoice Report'
		}
