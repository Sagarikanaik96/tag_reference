...
def email_notification(doc,method):
    ...
    if staffing[0]['tag_user_type'] == 'Staffing Admin' or staffing[0]['tag_user_type'] == 'Staffing User' or staffing[0]['tag_user_type'] == 'Administrator':
        try:
            ...
            make_system_notification(hiring_list,message=message,doctype=chat_message,docname=doc.name,subject=subject)
        ...
        try:
            ...
            job_order = frappe.db.sql(''' select  select_job from `tabJob Order` where name="{0}" '''.format(job[0]),as_dict=True)
            message=f"New Message regarding {job_order[0]['select_job']} from {staffing[0]['company']} is available."
            make_system_notification(staffing_list,message=message,doctype=chat_message,docname=doc.name,subject=subject)
