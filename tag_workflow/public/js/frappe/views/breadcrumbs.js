...
frappe.breadcrumbs = {
	...
	update() {
		let breadcrumbs = this.all[frappe.breadcrumbs.current_page()];
	...
	set_workspace(breadcrumbs) {
		...
		this.setup_modules();
		let from_module = this.get_doctype_module(breadcrumbs.doctype);
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		const docname = (frappe.get_route().toString().search("new-") > 0) ? "New "+frappe.get_route()[1] : frappe.get_route()[2];
	...
	rename(doctype, old_name, new_name) {
		let old_route_str = ["Form", doctype, old_name].join("/");
		let new_route_str = ["Form", doctype, new_name].join("/");
