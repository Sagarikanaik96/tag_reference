...
import Tab from "../../../../../../frappe/frappe/public/js/frappe/form/tab.js";
import Column from "../../../../../../frappe/frappe/public/js/frappe/form/column.js";
window.count = 0;
frappe.ui.form.Layout = class Layout {
  ...
  make() {
    ...
    this.message = $('<div class="form-message hidden"></div>').appendTo(
      this.wrapper
    );
  ...
  setup_tabbed_layout() {
    if (this.frm.doctype == "Company") {
      localStorage.setItem("slide_tab", 0);
      localStorage.setItem("slide_tab2", 0);
      $(`
      <div id="carouselExampleControls" class="carousel slide company-carousel" data-ride="carousel">
        <div class="carousel-inner">
          <div class="carousel-item active">
            <div class="form-tabs-list">
              <ul class="nav form-tabs" id="form-tabs1" role="tablist"> </ul>
            </div>
          </div>
          <div class="carousel-item">
            <div class="form-tabs-list">
              <ul class="nav form-tabs" id="form-tabs2" role="tablist"> </ul>
            </div>
          </div>
        </div>
        <button class="carousel-control-prev" id="pre-slide" type="button" data-target="#carouselExampleControls" data-slide="prev" >
          <i class="fa fa-angle-left" aria-hidden="true"></i>
          <span class="sr-only">Previous</span>
        </button>
        <button class="carousel-control-next" id="next-slide" type="button" data-target="#carouselExampleControls" data-slide="next" >
  ...
  replace_field(fieldname, df, render) {
    ...
    if (this.fields_dict[fieldname] && this.fields_dict[fieldname].df) {
      ...
      this.section.fields_list.splice(
        this.section.fields_dict[fieldname],
        1,
        fieldobj
      );
  ...
  make_tab(df) {
    ...
    if (this.frm.doctype == "Company") {
      window.count += 1;
      if (window.count > 9) {
        this.tabs_list = this.page.find("#form-tabs2");
      } else {
        this.tabs_list = this.page.find("#form-tabs1");
      }
    }
  ...
  refresh_fields(fields) {
    let fieldnames = fields.forEach((field) => {
