...
frappe.ui.Page = Class.extend({
  ...
  get_empty_state: function (title, message, primary_action) {
    let $empty_state;
    $empty_state = $(`<div class="page-card-container">
			<div class="page-card">
  ...
  setup_page: function () {
    ...
    if (
      ...
      window.location.pathname == "/app/job-order"
    ) {
      this.hiring_filter = $(`
					...
					<div class="filter-selector">
						...
						<script>
							function hiring_click(value){
								$('.page-form').find('select').each(function () {
									if($(this).attr('data-fieldname')=='order_status'){
										$(this).val(value);
										cur_list.refresh();
									}
								});
    ...
    if (
      ...
      (window.location.pathname == "/app/job-order" ||
        window.location.pathname == "app/job-order?data=direct")
    ) {
  ...
  setup_overlay_sidebar() {
    ...
    this.sidebar.on("click", "button:not(.dropdown-toggle)", () =>
      close_sidebar()
    );
  ...
  add_action_icon: function (icon, click, tooltip_label, css_class = "") {
    const button = $(`
  ...
  add_custom_button_group: function (label, icon, parent) {
    ...
    let custom_btn_group = $(`
			<div class="custom-btn-group toggle_list_view">
