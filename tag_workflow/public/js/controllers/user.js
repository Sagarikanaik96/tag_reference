...
function multiple_assign_properties(frm) {
  if (frm.doc.__islocal != 1) {
    ...
    frm.fields_dict["assign_multiple_company"].grid.wrapper
      .find(".grid-delete-row")
      .hide();
    cur_frm.fields_dict["assign_multiple_company"].grid.wrapper
      .find(".btn-open-row")
      .hide();
    cur_frm.fields_dict["assign_multiple_company"].grid.grid_rows.forEach(
      function (element) {
    ...
    if (old_password == new_password) {
      ...
      frappe.call({
        ...
        args: {
          ...
          old_password: cur_frm.doc.old_password,
        },
        callback: function (r) {
          if (!r.message) {
            frappe.validated = false;
