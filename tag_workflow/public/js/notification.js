...
class NotificationsView extends BaseNotificationsView {
	...
	get_dropdown_item_html(field) {
		...
		if(message.includes("has submitted a timesheet on")){
			message_html=`<div title="${field.document_name}" onclick="set_local(this.title)" class="message">
				<div>${message}</div>
				<div class="notification-timestamp text-muted">
					${timestamp}
				</div>
			</div>`;
			item_html = $(`<a class="recent-item notification-item ${read_class}" data-name="${field.name}">
				<div class="notification-body">
					${user_avatar}
					${message_html}
				</div>
...
function set_local(title){
	frappe.db.get_value("Timesheet",title,"job_order_detail",function(r){
		localStorage.setItem("order", r.job_order_detail);
